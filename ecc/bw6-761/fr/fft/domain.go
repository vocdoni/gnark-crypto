...
type Domain struct {
	...
	Twiddles, SplitTwiddles [][]fr.Element
...
func (d *Domain) preComputeTwiddles() {
	...
	// split twiddles
	innerLen := 1 << (bits.TrailingZeros64(d.Cardinality) / 2)
	outerLen := int(d.Cardinality) / innerLen
	nbRows := int(d.Cardinality) / outerLen
	d.SplitTwiddles = make([][]fr.Element, nbRows+1)
	g := d.Generator

	g.Exp(g, new(big.Int).SetInt64(int64(innerLen)))
	d.SplitTwiddles[0] = getSplitTwiddles(g, outerLen)

	for i := 0; i < nbRows; i++ {
		innerTwiddle := d.Twiddles[0][i]
		d.SplitTwiddles[i+1] = make([]fr.Element, outerLen)
		t := innerTwiddle
		for j := 0; j < outerLen; j++ {
			d.SplitTwiddles[i+1][j] = t
			t.Mul(&t, &innerTwiddle)
		}
	}
}
...
func getSplitTwiddles(root fr.Element, size int) []fr.Element {
	twiddles := make([]fr.Element, size/2)
	twiddles[0] = fr.One()
	for i := 1; i < len(twiddles); i++ {
		twiddles[i].Mul(&twiddles[i-1], &root)
	}
	BitReverse(twiddles)
	return twiddles
}
